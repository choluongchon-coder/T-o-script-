...
_G.AutoNhiemVuV4      = false -- CHỨC NĂNG 6 MỚI
...
-------------------------------------------------------------------
-- TẠO GIAO DIỆN THEO HÌNH VẼ "69c3ef96-25d2-4a3d-9c32-3aa63eaf8c07"
-------------------------------------------------------------------
...
MainFrame.Size = UDim2.new(0, 450, 0, 330) -- Tăng chiều cao để chứa thêm nút số 6
...
local function CreateFunctionRow(numText, nameText, callback)
    ...
    ToggleBox.Parent = RowFrame
    ToggleBox.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    ToggleBox.BorderColor3 = Color3.fromRGB(80, 80, 80)
    ToggleBox.BorderSizePixel = 1
    ToggleBox.Position = UDim2.new(1, -45, 0.5, -12)
    ...
    ToggleBox.MouseButton1Click:Connect(function()
        ...
        if isToggled then
            ...
            ToggleBox.BorderColor3 = Color3.fromRGB(255, 0, 0)
            ToggleBox.Text = "🤑"
        else
            ToggleBox.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            ToggleBox.BorderColor3 = Color3.fromRGB(80, 80, 80)
...
-------------------------------------------------------------------
-- LOGIC CHỨC NĂNG 6: TỰ ĐỘNG HOÀN THÀNH NV THEO TỪNG TỘC
-------------------------------------------------------------------
spawn(function()
    while task.wait(0.1) do
        if _G.AutoNhiemVuV4 then
            pcall(function()
                ...
                -- 1. TỘC THIÊN THẦN (Skypia) & TỘC THỎ (Mink): Bay thẳng tới đích
                if myRace == "Skypia" or myRace == "Mink" then
                    ...
                    if FinishPoint then
                        toTarget(FinishPoint.CFrame)
                    else
                        -- Tọa độ đích mặc định trong mê cung phòng Trial
                        toTarget(CFrame.new(28850, 14920, -250)) 
                    end

                -- 2. TỘC NGƯỜI (Human) & TỘC QUỶ (Ghoul): Đánh sạch quái phòng Trial
                elseif myRace == "Human" or myRace == "Ghoul" then
                    for _, mob in pairs(Workspace.Enemies:GetChildren()) do
                        if mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:FindFirstChild("HumanoidRootPart") then
                            repeat
                                task.wait()
                                myHrp.CFrame = mob.HumanoidRootPart.CFrame * CFrame.new(0, 8, 0) -- Bay trên đầu quái né đòn
                                -- Spam sát thương X2
                                for i = 1, 2 do
                                    local tool = player.Character:FindFirstChildOfClass("Tool")
                                    if tool then 
                                        tool:Activate() 
                                        game:GetService("ReplicatedStorage").Remotes.Validator:FireServer(tool.Name, mob.HumanoidRootPart.Position)
                                    end
                                end
                ...
                -- 3. TỘC CÁ (Fishman): Bay lên mặt biển, tự động dùng võ/trái ác quỷ diệt Sea Beast
                elseif myRace == "Fishman" then
                    -- Di chuyển lên mặt nước (Sea) để làm nhiệm vụ
                    myHrp.CFrame = CFrame.new(myHrp.Position.X, 150, myHrp.Position.Z)
                    
                    -- Tìm Sea Beast hoặc Quái biển trong Trial
                    for _, seaMob in pairs(Workspace.Enemies:GetChildren()) do
                        if string.find(seaMob.Name, "Sea") and seaMob:FindFirstChild("HumanoidRootPart") then
                            -- Tự chọn vũ khí/Võ/Trái ác quỷ để dùng chiêu thức
                            local tool = player.Character:FindFirstChildOfClass("Tool")
                            if tool then
                                tool:Activate()
                                -- Tự động tung các chiêu thức từ xa (Z, X, C, V) của Võ hoặc Trái
                ...
                -- 4. TỘC NGƯỜI MÁY (Cyborg): Bay lên nóc trần phòng né bom
                elseif myRace == "Cyborg" then
                    -- Bay thẳng lên cao, áp sát nóc trần phòng Trial để né mọi vụ nổ bom ở dưới đất
