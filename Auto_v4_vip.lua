-- =========================================================================
-- SCRIPT: ALL-IN-ONE BLOX FRUIT (FIX TRÚT GIẬN - KILL AURA 100% CHẠY)
-- CHỨC NĂNG: Sửa Kill Aura Đập Quái, Ép Bay Tween 310, Menu Góc Dưới Phải
-- =========================================================================
...
-------------------------------------------------------------------
-- 1. HÀM TRANG BỊ VŨ KHÍ & BAY TWEEN
-------------------------------------------------------------------
local function EquipMelee()
    local char = player.Character
    if not char then return end
    
    -- Kiểm tra nếu chưa cầm vũ khí Melee trên tay
    local currentTool = char:FindFirstChildOfClass("Tool")
    if currentTool and (currentTool:FindFirstChild("Melee") or currentTool.ToolTip == "Melee") then
        return currentTool
    end
    
    -- Tìm trong balo và lôi ra tay bằng được
...
-- Vòng lặp bật xuyên tường liên tục
...
-------------------------------------------------------------------
-- 2. LUỒNG REWRITE KILL AURA MỚI (KHÔNG LO LỖI KHÔNG ĐẬP)
-------------------------------------------------------------------
spawn(function()
                    ...
                    if folderMobs then
                        for _, v in pairs(folderMobs:GetChildren()) do
                            if v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") then
                                ...
                                -- Tầm quét Kill Aura tăng lên 70 block quanh nhân vật
                                if dist < 70 then
                                    -- 1. Ép cầm vũ khí
                                    local weapon = EquipMelee()
                                    
                                    -- 2. Đóng băng quái không cho văng ra xa do Knockback
                                    v.HumanoidRootPart.Velocity = Vector3.new(0,0,0)
                                    
                                    -- 3. Spam click trực tiếp qua hệ thống ảo của Game
                                    VirtualUser:ClickButton1(Vector2.new(850, 520)) 
                                    if weapon then
                                        weapon:Activate()
                                        -- Gửi kèm Validator gói tin đập trực diện lên Server
                                        ReplicatedStorage.Remotes.Validator:FireServer(weapon.Name, v.HumanoidRootPart.Position)
                                    end
                                end
                            end
                        end
...
-------------------------------------------------------------------
-- 3. LUỒNG LOGIC DI CHUYỂN (TWEEN 310)
-------------------------------------------------------------------
        ...
        if _G.AutoRipIndri then
            pcall(function()
                ...
                if RipMonster and RipMonster:FindFirstChild("Humanoid") and RipMonster.Humanoid.Health > 0 and RipMonster:FindFirstChild("HumanoidRootPart") then
                    ...
                    toTarget(RipMonster.HumanoidRootPart.CFrame * CFrame.new(0, 7.5, 0))
                else
                    _G.IsFightingRip = false
                end
            end)
        ...
        if _G.AutoFarmLvKemBatV4 and not _G.IsFightingRip then
            pcall(function()
                ...
                if not player.PlayerGui.Main.Quest.Visible then
                    ...
                    if (player.Character.HumanoidRootPart.Position - targetCFrame.Position).Magnitude < 20 then
                ...
                if MobInZone then
                    toTarget(MobInZone.HumanoidRootPart.CFrame * CFrame.new(0, 7.5, 0))
                else
            ...
            pcall(function()
                ...
                for _, p in pairs(game.Players:GetPlayers()) do
                    if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then
                        toTarget(p.Character.HumanoidRootPart.CFrame * CFrame.new(0, 7, 0))
                    end
                end
            end)
        end
...
-- 4. BAY ĐẾN ĐỈNH CÂY VÀO DOOR V4
spawn(function()
    while task.wait(0.2) do
        if _G.BayDenDoor and not _G.IsFightingRip then
            pcall(function()
                ...
                if (player.Character.HumanoidRootPart.Position - DinhCayCFrame.Position).Magnitude < 35 then
    ...
    while task.wait(0.2) do
        if _G.BayDenDongHo and not _G.IsFightingRip then
            pcall(function()
                ...
                if (player.Character.HumanoidRootPart.Position - ClockCFrame.Position).Magnitude < 35 then
                    ...
                    if folderMobs then
                        for _, mob in pairs(folderMobs:GetChildren()) do
                            if mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:FindFirstChild("HumanoidRootPart") then
                                toTarget(mob.HumanoidRootPart.CFrame * CFrame.new(0, 7.5, 0))
                            end
                        end
                    end
...
-------------------------------------------------------------------
-- 4. HỆ THỐNG GIAO DIỆN (MENU GÓC DƯỚI BÊN PHẢI)
-------------------------------------------------------------------
local ScreenGui = Instance.new("ScreenGui")
pcall(function() ScreenGui.Parent = game:GetService("CoreGui") end)
if not ScreenGui.Parent then ScreenGui.Parent = player:WaitForChild("PlayerGui") end
ScreenGui.Name = "HydraDat_TabGUI_BottomRight"

local MainFrame = Instance.new("Frame", ScreenGui)
MainFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
MainFrame.BorderColor3 = Color3.fromRGB(255, 0, 0)
MainFrame.BorderSizePixel = 2
MainFrame.Position = UDim2.new(1, -470, 1, -420) 
MainFrame.Size = UDim2.new(0, 450, 0, 380)
MainFrame.Active = true

-- Hệ thống kéo thả Menu tự do bằng chuột/tay
...
ToggleMenuButton.Position = UDim2.new(1, -60, 1, -450)
